...
const processRawConstellation = con => {
    ...
    const nodes = split[4].split('[')[1].split('center').slice(1, split[4].length).map(x => x.split(':')).map(y=> y.slice(2, 4)).map(z => [z[0].split(',')[0] ,z[1].split('}')[0] ])

    const processedNodes = nodes.map((n, idx) => {return {star_id: idx, x: parseInt(n[0]), y: parseInt(n[1])}})
    
    const lines = split[5].split('source').slice(1, split[5].length).map(x=> x.split(",").slice(0, 2)).map(y=>[y[0].split(':')[1], y[1].split(":")[1].split("}")[0]])

    const processedLines = lines.map((n, idx) => {return {line_id: idx, to: parseInt(n[0]), from: parseInt(n[1])}})
    ...
    const fullyProcessedLines = processedLines.map( n => {
        ...
        return {line_id: n.line_id, to: {x: to_star.x, y: to_star.y} , from: {x: from_star.x, y: from_star.y}}
    })
